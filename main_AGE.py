...
def fcv_5(full_set, n_examples, part_lengths,atributes,i,set_type,seed):
      ...
      train_set, test_set, train_set_size, test_set_size, train_data, test_data,train_labels, test_labels = utilities.select_partition(full_set, n_examples, part_lengths, i,set_type,atributes)
      ...
      start = time.time()
      poblacion, mejor_fit, c_mejor, r_mejor, indice_mejor, cromosoma = genetico.AGE_BLX(train_data,train_labels,weights,atributes)
      end = time.time()   

      print("Poblacion: ",poblacion) 
      print("Mejor valor fitness: ",mejor_fit,"Tasa de clasificación: ",c_mejor," Tasa de reduccion: ",r_mejor," Indice: ",indice_mejor," Cromosoma: ",cromosoma)
      print("Tiempo de ejecucion AGE BLX_alpha: ",end-start)
      print("Probamos en conjunto test: ")
      f_test, c_test, r_test = weighted_fitness_function(test_data,labels,cromosoma)
      print("Mejor valor fitness: ",f_test," Tasa de clasificación: ",c_test," Tasa de reduccion: ",r_test)
      
      start = time.time()
      poblacion, mejor_fit, c_mejor, r_mejor, indice_mejor,cromosoma = genetico.AGE_aritmetico(train_data,train_labels,weights,atributes)
      end = time.time()   
