...
def class_rate(data, labels, w):
    #print("HOLA")
    #print("Class rate")
    dist = distance.squareform(distance.pdist(data, w=w))
    #print("dist",dist)
    nrows = data.shape[0]
    ii = np.arange(nrows)
    dist[ii,ii] = np.inf 
    ...
    #print(dist[0])
    #da igual el eje porque la matriz dist es simétrica
    selected = dist.argmin(axis=1)
    #print("selected:",selected)
    #print(labels[selected])
    #print(labels)
    #print("Prueba: data[0]",data[0]," data[154]",data[154])
    #print("Tasa de aciertos: ",(labels[selected] == labels).sum()/len(labels))
    return (labels[selected] == labels).sum()/len(labels)

def reduction_rate(w):
    return (w <= 0.1).sum()/len(w)
...
def processData(ficheros,s):
    ...
    for i in range(len(meta.names())-1):
        atributes.append(meta.names()[i])

    #print(dataset)
    #print(atributes)

    n_atributes = len(atributes)-1
    ...
    final_set = np.empty(shape = (n_examples),dtype= s)
    ...
    for atribute in atributes:
        #print(final_set[atribute])
        #rescaled_weight = scaler.fit_transform(final_set[atribute])
        #print(rescaled_weight)
        final_set[atribute]= preprocessing.minmax_scale(final_set[atribute])

    #for atribute in atributes:
        #print(final_set[atribute])
    #print("ATRIBUTOS QUE VAMOS A DEVOLVER: ",atributes)
    #print(final_set)
    return [atribute for atribute in atributes],final_set, n_examples, part_lengths
#print(final_set)

##############atributes,final_set,n_examples,part_lengths = processData(ozone)
#print("ATRIBUTOS DEVUELTOS: ",atributes)
"""
...
def select_partition(full_set, n_examples, part_lengths, i,set_type,atributes):
    ...
    ### control ###
    #catch(full_set, n_examples, atributes)

    train_set = np.empty(shape = (n_examples - part_lengths[i]),dtype=s)
    test_set = np.empty(shape = (part_lengths[i]),dtype=s)
    ...
    for j in range(len(part_lengths)):
        ...
        if (j > i):
            ...
            for k in range(part_lengths[j]):
                ...
                for atribute in atributes: 
                    #print("fila :",atribute, "columna :",test_start_pointer + test_set_size -1 + k)
                    train_set[atribute][last_pos_train + k] = full_set[atribute][ last_pos_train + test_start_pointer + test_set_size -1 + k]
    ...
    #catch(train_set,train_set_size, atributes)
    #catch(test_set,test_set_size,atributes)
    #print("Train set. Size =  ",train_set_size,train_set)
    #print("Test set. Size =  ",test_set_size,test_set)
    n_atributes = len(atributes)
    #print("Probando columnas.")
    #print(train_set)
    """
    print(type(train_set))
    print(type(train_set[0][0]))
    aux = train_set[0].tolist()
    aux_2 = np.array(aux)
    print(len(aux_2))
    print(type(aux_2))
    print(aux_2)
    """
    train_data = np.empty(shape = (train_set_size,n_atributes))
    train_labels = np.empty(shape = train_set_size,dtype = np.dtype('U15'))

    for i in range(train_set_size):
        for j in range(n_atributes):
            train_data[i][j] = train_set[i][j]
        train_labels[i] = train_set[i][-1]


    test_data = np.empty(shape = (test_set_size,n_atributes))
    test_labels = np.empty(shape = test_set_size,dtype = np.dtype('U15'))

    for i in range(test_set_size):
        for j in range(n_atributes):
            test_data[i][j] = test_set[i][j]
        test_labels = test_set[i][-1]
    #print(test_data)
    #for i in range(train_set_size):
        #train_data[i] = train_set[0][0]
        #print(train_set[:, :-1])
    #hola(test_data,test_labels,np.random.rand(n_atributes))
    #print("Train set")
    #for i in range(train_set_size):
        #print(train_set[i])
    return train_set, test_set, train_set_size, test_set_size, train_data, test_data, train_labels, test_labels
...
def weighted_1NN(data,labels,w):
...
def fitness_function(train_set,test_set,test_set_size,clasificacion):
    ...
    return f_f, c_r, r_r
"""
def weighted_fitness_function(data,labels,w):
    c_r = class_rate(data,labels,w)
    r_r = reduction_rate(w)

      #print("TASA REDUCCION")
    f_f = alpha*c_r + (1-alpha)*r_r
    #print("Ante los datos: ",data,w)
    #print("fitness function para",w," va a devolver: f_f ",f_f," c_r ",c_r," r_r ",r_r)
